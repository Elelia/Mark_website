...
export default function AddPage() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const categoriesFResponse = await axios.get('http://192.168.1.72:5000/seriefilm/film/categories');
                setCategFilm(categoriesFResponse.data);

                const categoriesSResponse = await axios.get('http://192.168.1.72:5000/seriefilm/serie/categories');
    ...
    const searchFilm = async (event) => {
        ...
        try {
            const filmsCategories = await axios.get(`http://192.168.1.72:5000/seriefilm/film/get_tmdb/${selectedCategFilm}`);
    ...
    const searchSerie = async (event) => {
        event.preventDefault();
        try {
            const seriesCategories = await axios.get(`http://192.168.1.72:5000/seriefilm/serie/get_tmdb/${selectedCategSerie}`);
    ...
    const validateMovie = async (rows) => {
        try {
            for(let i=0; rows.length > i; i++) {
                ...
                await axios.post(`http://192.168.1.72:5000/seriefilm/film/insertMovie`, {
                    id_movie
                });
            }
    ...
    const validateSerie = async (rows) => {
        try {
            for(let i=0; rows.length > i; i++) {
                ...
                await axios.post(`http://192.168.1.72:5000/seriefilm/serie/insertSerie`, {
                    id_serie
                });
            }
            alert("Enregistrement en base réussi.");
